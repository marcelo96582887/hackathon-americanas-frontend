...
import { FiPlus, FiXSquare, FiX } from 'react-icons/fi';
...
export default function CadastroUsuario(props) {
  ...
  const [errornomeLoja, setErrorNomeLoja] = useState('');
  ...
  const [errornomeUsuario, setErrorUsuario] = useState('');
  ...
  if (props.location.state?.init && !init) {
    setInit(true);
    const { parametros } = props.location.state;
  ...
  async function handleInsert() {
    ...
    try {
      const response = api.post('produtos', data);
      setInputCodigo('');
      setInputProduto('');
      setInputDescricao('');
      setInputSaldo('');
